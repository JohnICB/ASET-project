...
def train():
    ...
    early_stopping = EarlyStopping(monitor=const.MONITOR, min_delta=const.MIN_DELTA, patience=patience_early,
                                   restore_best_weights=True, verbose=1)

    logdir = nn.fn_logger + "/" + datetime.now().strftime("%Y%m%d-%H%M%S")
    logger = keras.callbacks.TensorBoard(log_dir=logdir)
    ...
    while True:
        loop += 1
        h = nn.model.fit(
            x=nn.prepare_data(images, labels),
            shuffle=True,
